...
class RhythmFormer(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(1, 2)  #input[B,C,T,H,W]->[B,T,C,H,W]
        N, D, C, H, W = x.shape
...
if __name__ == '__main__':
    input = torch.randn((1, 3, 160, 128, 128))  # , dtype=torch.float32
    model = RhythmFormer()
    ...
    print('flops: %.2f G, params: %.2f M' % (flops / 1000000.0 / 1024, params / 1000000.0))

    # torch.cuda.reset_peak_memory_stats()
    # device = torch.device('cuda:0')
    # input = torch.randn(1, 3, 120, 128, 128).to(device)
    # module = RhythmFormer().to(device)  # Fusion_Stem()
    # # with torch.no_grad():
    # _ = module(input)
    #
    # mem_usage = torch.cuda.max_memory_allocated() / 1024 / 1024  # 转 MB
    # print(f"GPU Memory Usage: {mem_usage:.2f} MB")