...
class TSCAN(nn.Module):
    ...
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
                 dropout_rate2=0.5, pool_size=(2, 2), nb_dense=128, frame_depth=20, img_size=128):
    ...
    def forward(self, inputs, params=None):
        ...
        inputs = inputs.view(N * D, C, H, W)  # 合并 batch 维度
        raw_input = inputs
        # 复制最后一帧，防止 torch.diff 造成时间步减少
        last_frame = torch.unsqueeze(inputs[-1, :, :, :], 0)  # (1, C, H, W)
        inputs = torch.cat((inputs, last_frame), dim=0)  # 在第 0 维拼接
        inputs = torch.diff(inputs, dim=0)

        diff_input = self.batch_norm(inputs)  # [nt, c, h, w], 2*160, 3, 128, 128
        ...
        g2 = torch.sigmoid(self.apperance_att_conv2(r6))  # [320, 1, 61, 61]
        g2 = self.attn_mask_2(g2)                         # [320, 1, 61, 61]
        gated2 = d6 * g2                                  # [320, 64, 61, 61]

        d7 = self.avg_pooling_3(gated2)                   # [320, 64, 30, 30]
        d8 = self.dropout_3(d7)                           # [320, 64, 30, 30]
...
if __name__ == '__main__':
    ...
    input = torch.randn((2, 3, 160, 128, 128))  # , dtype=torch.float32
    model = TSCAN()
